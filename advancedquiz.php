...
<head>
    ...
    <title>HyperLingual | Advanced Quiz</title>
...
if (isset($_GET['quiz'])) {
    $quiz = $_GET['quiz'];
    $lang = substr($quiz, 0, strpos($quiz, 'Advanced'));
    $level = substr($quiz, strpos($quiz, 'Advanced'));
    ...
    echo '<form method="post" action="advancedverify.php?lang=' . $lang . '&level=' . $level . '">';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Input fields for the user's answers (three inputs per question)
        echo '<input type="text" name="answer['.$question_id.'][1]" required class="input-box"><br>';
        echo '<input type="text" name="answer['.$question_id.'][2]" required class="input-box"><br>';
        echo '<input type="text" name="answer['.$question_id.'][3]" required class="input-box"><br>';
        $i++;   
    }
