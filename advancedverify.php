...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($answers as $question_id => $submitted_answers) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $correct_answers[] = strtolower(trim($row['answer_text']));
        }

        // Sort the submitted answers and correct answers for comparison
        sort($submitted_answers);
        sort($correct_answers);

        // Verify if the submitted answers match the correct answers
    ...
    if(isset($_GET['level'])) {
        $level = $_GET['level'];
        $lang = $_GET['lang'];
        if ($level == 'Advanced' && $lang == 'English'){
            $quiz_id = 3;
        } else if ($level == 'Advanced' && $lang == 'Japanese'){
            $quiz_id = 6;
        }
        ...
        if ($num_attempts > 0) {
            ...
            foreach ($answers as $question_id => $submitted_answers) {
                ...
                // Verify if the submitted answers match the correct answers
                $is_correct = ($submitted_answers === $correct_answers) ? 1 : 0;
...
<head>
    ...
    <title>HyperLingual | Advanced Quiz Result</title>
    <style>
