...
<head>
    ...
    <title>HyperLingual | Intermediate Quiz</title>
...
if (isset($_GET['quiz'])) {
    $quiz = $_GET['quiz'];
    $lang = substr($quiz, 0, strpos($quiz, 'Intermediate'));
    $level = substr($quiz, strpos($quiz, 'Intermediate'));
    ...
    echo '<form method="post" action="intermediateverify.php?lang=' . $lang . '&level=' . $level . '">';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<input type="text" name="answer['.$question_id.']" required class="input-box"><br>';
